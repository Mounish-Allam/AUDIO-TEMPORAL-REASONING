...
def demo_data():
    ...
    # After running scripts/prepare_audiocaps.py these will be populated.
    # Fall back to a minimal format check so the demo still passes before download.
    train_path = "data/train.json"
    test_path  = "data/test.json"
    pref_path  = "data/preference_pairs.json"

    assert os.path.exists(train_path), (
        f"{train_path} not found — run: python scripts/prepare_audiocaps.py"
    )
    assert os.path.exists(test_path), (
        f"{test_path} not found — run: python scripts/prepare_audiocaps.py"
    )
    assert os.path.exists(pref_path), (
        f"{pref_path} not found — run: python scripts/prepare_audiocaps.py"
    )
...
def demo_outputs():
    ...
    assert report, "outputs/benchmark_report.json not found"
    assert preds,  "outputs/predictions.json not found"
