"""
Generate predictions.json + test.json from AudioCaps test split.

Downloads AudioCaps test metadata from HuggingFace (no audio files needed).
Simulates three model stages so the dashboard shows realistic comparison data:

  Base model  -> short, vague, often hallucinated descriptions
  After SFT   -> better temporal structure, fewer hallucinations
  After DPO   -> close to ground truth, correct temporal ordering

Usage:
    python scripts/generate_audiocaps_predictions.py
    python scripts/generate_audiocaps_predictions.py --max_samples 200 --stage dpo
"""
...
def main():
    ...
    print("=" * 60)
    print(f"  AudioCaps Predictions Generator")
    ...
    sep = "-" * 44
    print(f"\n  Generated {len(predictions)} predictions  ->  {pred_path}")
